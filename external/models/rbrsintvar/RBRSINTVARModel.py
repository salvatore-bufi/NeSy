...
class RBRSINTVARModel(torch.nn.Module, ABC):
    ...
    def forward(self, inputs, **kwargs):
        ...
        xui = self.disjunction(1.0, and_scores_tensor)
    ...
    def compute_kl_loss(self, mu, logvar):
        mu_exp = mu.unsqueeze(1).expand(-1, self.n_rules, -1)
        kl_divergence = -0.5 * torch.sum(1 + logvar - mu_exp.pow(2) - logvar.exp(), dim=[1, 2])
    ...
    def train_step(self, batch):
        ...
        # KL Divergence Loss
        kl_loss = self.compute_kl_loss(gu_mean_pos, gu_logvar_pos)
